...
if ( ! function_exists( 'aster_storefront_categories_list' ) ) :
	...
	function aster_storefront_categories_list( $with_background = false ) {
		if ( 'post' === get_post_type() ) {
			$hide_category = get_theme_mod( 'aster_storefront_post_hide_category', false );
...
if ( ! function_exists( 'aster_storefront_entry_footer' ) ) :
	...
	function aster_storefront_entry_footer() {
		...
		if ( 'post' === get_post_type() && is_singular() ) {
			$hide_tag = get_theme_mod( 'aster_storefront_post_hide_tags', false );

			if ( ! $hide_tag ) {
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'aster-storefront' ) );
				if ( $tags_list ) {
					/* translators: 1: list of tags. */
					printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'aster-storefront' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
		}
